...
------------------------
This module sets BLOG_DB_PATH / PORTFOLIO_IMAGE_DIR / CONTENT_IMAGE_DIR /
ADMIN_USERNAME / ADMIN_PASSWORD environment variables to point at a single
temporary directory *before* backend/db.py and backend/app.py are imported
for the first time (those modules read the env vars once, at import time,
exactly like the hardcoded defaults they replace -- see backend/db.py and
the PORTFOLIO_IMAGE_DIR/CONTENT_IMAGE_DIR comment in backend/app.py). That
means:

- The real backend/blog.db is never opened by the test suite.
- The real static/images/portfolio/ and static/images/uploads/ directories
...
_TEST_CONTENT_IMAGE_DIR = _TEST_ROOT / "content_uploads"
_TEST_CONTENT_IMAGE_DIR.mkdir(parents=True, exist_ok=True)
...
os.environ["CONTENT_IMAGE_DIR"] = str(_TEST_CONTENT_IMAGE_DIR)
...
def _clear_uploaded_images():
    for directory in (_TEST_UPLOAD_DIR, _TEST_CONTENT_IMAGE_DIR):
...
@pytest.fixture()
def content_upload_dir():
    """The temp directory /admin/upload-image saves inline Markdown images
    to during tests, for assertions that a file was written to disk."""
    return _TEST_CONTENT_IMAGE_DIR
